...
export const Hero: React.FC<HeroProps> = ({ onExploreInventory, onContactClick }) => {
  return (
    <section
      id="hero"
      className="relative h-screen min-h-[700px] flex flex-col justify-between pt-28 sm:pt-36 pb-12 sm:pb-16 px-6 sm:px-12 lg:px-20 bg-[#070707] overflow-hidden"
    >
      {/* Dominant Cinematic Automotive Imagery (Close, large, atmospheric) */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        ...
        >
          <img
            ...
            className="w-full h-full object-cover object-[center_38%] sm:object-[center_32%] filter contrast-[1.12] brightness-[0.88] saturate-[0.9]"
          />
          {/* Subtle cinematic gradient: Vehicle remains clearly visible, typography remains sharp */}
          <div className="absolute inset-0 bg-gradient-to-t from-[#070707] via-[#070707]/35 to-[#070707]/70" />
          <div className="absolute inset-0 bg-gradient-to-r from-[#070707]/80 via-transparent to-transparent hidden sm:block" />
        </motion.div>
      </div>
      ...
      {/* Main Campaign Headline & Actions (No paragraphs, no clutter) */}
      <div className="relative z-20 w-full my-auto py-8">
        ...
        >
          <button
            ...
            className="group inline-flex items-center space-x-3 text-[11px] tracking-[0.28em] uppercase text-[#F3F0E8] hover:text-white px-6 py-3.5 border border-white/[0.15] hover:border-[#B99A5B] transition-all duration-300 cursor-pointer bg-[#070707]/40 backdrop-blur-sm"
          >
      ...
      {/* Baseline: Clean location signature */}
      <div className="relative z-20 w-full pt-6 border-t border-white/[0.08] flex items-center justify-between text-[10px] tracking-[0.32em] uppercase text-[#8A877F]">
        ...
        <span className="font-mono text-[#8A877F]/60">EST. 2021</span>
      </div>
    </section>
  );
};
